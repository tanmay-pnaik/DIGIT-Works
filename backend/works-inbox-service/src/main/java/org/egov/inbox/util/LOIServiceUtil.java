...
public class LOIServiceUtil {
    ...
    public List<String> fetchLOINumbersFromLOIService(InboxSearchCriteria criteria, HashMap<String, String> StatusIdNameMap, RequestInfo requestInfo) {
        ...
        if (!isSearchResultEmpty) {
            ...
            if (moduleSearchCriteria.containsKey(LOIConstant.LOI_IDS)) {
                uriBuilder.append("&ids=").append(moduleSearchCriteria.get(LOIConstant.LOI_IDS));
            }
            ...
            loiNumbers = JsonPath.read(result, "$.estimates.*.estimateNumber");
